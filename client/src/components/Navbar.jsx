            ...
            <>
              ...
              <Link to="/profile" className="nav-profile" title={auth.user.name} aria-label="Profile">
                {auth.user.avatar_url && pfpOk ? (
                  <img src={auth.user.avatar_url} alt="avatar" className="nav-avatar" onError={() => setPfpOk(false)} />
                ) : (
...
function NotifText({ n }) {
  ...
  if (t === 'new_post_by_followed_author') return <span>New post: <Link to={`/article/${p.article_id}`}>{p.title || 'Article'}</Link></span>
